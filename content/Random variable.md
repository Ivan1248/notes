...
$$  
    \forall X'\in \Sigma'\colon \rvar x^{-1}(X')\in \Sigma \text.  
$$
...
Expressions such as $f(\rvar x_1, \rvar x_2, .., \rvar x_n)$, where $f$ represents a function, are shorthand for the function $\omega \mapsto f(\rvar x_1(\omega), \rvar x_2(\omega), .., \rvar x_n(\omega)) \colon \Omega\to \operatorname{im}(f)$.
...
Exceptions are "random-variable-aware" [[Functional|functionals]], such as the [[Probability measure|probability measure]] when denoted with $\P$ and [[Expectation|expectation]].  
