...
- The **set of all possible labels** is a [[Finite set|finite set]] $Y$.
...
**Early action prediction** is a [[Machine learning task|machine learning task]] where the training set is $D_\text{train}$, and the model is [[Machine learning evaluator|evaluated]] on predicting $y$ given $\v x'$, where $(\v x', y) \in D_{\text{test}}$.
